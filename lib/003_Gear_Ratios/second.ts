...
const match_char_with_numbers_curated = (
  char: MatchingObj,
  curated_numbers: MatchingObj[]
): MatchingObj[] => {
  const filtered_numbers = curated_numbers.filter((obj) => {
    if (
      obj.line === char.line - 1 ||
      obj.line === char.line ||
      obj.line === char.line + 1
    ) {
      if (
        obj.num_index === char.num_index - 1 ||
        obj.num_index === char.num_index ||
        obj.num_index === char.num_index + 1
      ) {
        return obj;
      }
  ...
  return filtered_numbers.length == 2 ? filtered_numbers : [];
};
...
matching_numbers_curated = chars.flatMap((char) =>
  match_char_with_numbers_curated(char, matching_numbers_curated)
);
...
for (let i = 0; i < numbers.length; i = i + 2) {
  power += Number(numbers[i]) * Number(numbers[i + 1]);
}
